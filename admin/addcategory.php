<?php
session_start();
include("includes/db.inc.php");

if(isset($_POST['btn_save'])) 
{
// add the new category to the database

$cat_title=$_POST['cat_title'];
$cat_path=$_POST['cat_path'];
$conn->query("insert into category(`cat_title`,`cat_path`) values('$cat_title','$cat_path')")or die("Query 2 is inncorrect..........".$conn->error());
header("location: category.php");
$conn->close();
}
// sql statemant is done
include "sidenav.php";
include "topheader.php";
?>
<!-- End Navbar -->
<div class="content">
    <div class="container-fluid">
        <div class="col-md-5 mx-auto">
            <div class="card">
                <div class="card-header card-header-primary">
                    <!-- header title -->
                    <h5 class="title">Add Category</h5>
                </div>
                <form action="addcategory.php" name="form" method="post" enctype="multipart/form-data">
                    <div class="card-body">

                        <div class="col-md-12 ">
                            <div class="form-group">
                                <label>category title</label>
                                <input type="text" id="cat_title" name="cat_title" class="form-control">
                            </div>
                        </div>
                        <div class="col-md-12 ">
                            <div class="form-group">
                                <label>category path</label>
                                <input type="text" id="cat_path" name="cat_path" class="form-control">
                            </div>
                        </div>
                    </div>
                    <div class="card-footer">
                        <button type="submit" id="btn_save" name="btn_save"
                            class="btn btn-fill btn-primary">Add</button>
                    </div>
                </form>
            </div>
        </div>


    </div>
</div>
<?php
include "footer.php";
?>